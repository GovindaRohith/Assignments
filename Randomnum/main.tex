...
%
and plot its CDF.  \\
\solution The CDF is as follows 
\solution Download the following files and execute the  C program.
\begin{lstlisting}
...
\begin{align}
    V&=-2\ln{(1-U)}\\
    \implies E(V)&=-2\ln({1-E(U)})\\
\int_{-\infty}^{x}xp_V(x)dx&=-2\ln{(1-\frac{x^2}{2})}
\end{align}
Differentiating the above equation 
\begin{align}
    p_{V}(x)&=\frac{4}{2-x^2}\\
    \implies F_{V}(x)&=\int_{0}^{x}\frac{4dx}{2-x^2}\\
    \boxed{F_{V}(x)=\sqrt{2}\ln{\frac{\sqrt{2}+x}{\sqrt{2}-x}}}
\end{align}
%\item
